# MEMORY.md - 项目长期记忆

## 网站部署
- **GitHub 仓库**: zhangaliluya (public)
- **Cloudflare Pages**: zhangaliluya.pages.dev
- **域名**: aliluya.asia（阿里云购买）
- **Cloudflare nameservers**: bob.ns.cloudflare.com / riya.ns.cloudflare.com（注意不是通用的 ns1/ns2！）
- DNS 已生效，aliluya.asia 已添加到 Cloudflare Pages 自定义域名，等待 SSL 验证

## 风格修改记录（2026-06-02）
- 背景色改为纯黑 `#0a0a0a`（原为浅灰 `#e5e7eb`）
- Hero 区域背景视频已移除，改为纯黑背景
- 已备份原始文件为 `index-backup.html`

## 首页结构（当前）
1. Hero: Hi~ I'm 阿莉路亚
2. Manifesto: Design that works. Results that last.
3. Portfolio Selection 展示区（四角标签、三栏排版、底部括号）
4. NenoVia 瀑布流图片区（4张卡片扇形展开，圆角16px）
5. Footer
- About 区块已删除

## 设计偏好
- 主色调: #2a43ad（深蓝）
- 圆角风格: 16px
- 字体: 大写英文、紧凑字间距
